...
async function loadStudents() {
    ...
    try {
        const response = await fetch(`../api/students.php?action=list&search=${search}&status=${status}&course=${course}&payment=${payment}&page=${currentPage}`);
        ...
        <tr class="border-b border-slate-100 hover:bg-slate-50 transition-colors">
            ...
            <td class="px-6 py-4">
                <div class="flex items-center gap-3">
                    <img src="${student.photo || '../assets/default-avatar.png'}" 
...
async function viewStudent(studentId) {
    try {
        const response = await fetch(`../api/students.php?action=get&id=${studentId}`);
        const result = await response.json();
