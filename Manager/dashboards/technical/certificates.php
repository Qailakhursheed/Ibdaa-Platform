...
try {
    ...
    $stmt->execute();
    $certsStats['total_certs'] = $stmt->fetch(PDO::FETCH_ASSOC)['total'];
    
    // Issued Certificates
    $stmt = $conn->prepare("SELECT COUNT(*) as total FROM certificates WHERE status = 'issued'");
    $stmt->execute();
    $certsStats['issued_certs'] = $stmt->fetch(PDO::FETCH_ASSOC)['total'];
    
    // Pending Certificates
    $stmt = $conn->prepare("SELECT COUNT(*) as total FROM certificates WHERE status = 'pending'");
    $stmt->execute();
    $certsStats['pending_certs'] = $stmt->fetch(PDO::FETCH_ASSOC)['total'];
    ...
    ");
    $stmt->execute();
    $certsStats['this_month'] = $stmt->fetch(PDO::FETCH_ASSOC)['total'];
...
async function loadCertificates(courseId = 'all', status = 'all', searchTerm = '') {
    try {
        let url = '../../api/certificates.php?action=get_all';
...
async function viewCertificate(certId) {
    try {
        const response = await fetch(`../../api/certificates.php?action=get_details&certificate_id=${certId}`);
        const data = await response.json();
...
function downloadCertificate(certId) {
    window.open(`../../api/certificates.php?action=download&certificate_id=${certId}`, '_blank');
}

function printCertificate(certId) {
    window.open(`../../api/certificates.php?action=print&certificate_id=${certId}`, '_blank');
}

// Load Students
async function loadStudentsForCert() {
    try {
        const response = await fetch('../../api/students.php?action=get_all');
        const data = await response.json();
...
async function loadStudentCourses(studentId) {
    try {
        const response = await fetch(`../../api/students.php?action=get_courses&student_id=${studentId}`);
        const data = await response.json();
